...
from datetime import datetime #現在時刻等の時間を扱う機能をインポート
...
current_city_code = st.empty() #選択中の地域を補給時するための箱をcurrent_city_codeとして用意
current_city_code.write("選択中の地域:" + city_code_index) #用意した箱に選択した地域を代入し表示させる
...
weather_json = response.json() #responseにjson形式の天気データが返ってくるので、response.json()をwether_jsonに代入
...
# 天気の情報を0-6時、6-12時、12-18時、18-24時の4つに分けて降水確率を今日、明日、明後日の3日間の天気を返すため、場合分けをする。
...
df3 = pd.DataFrame(weather_json['forecasts'][2]['chanceOfRain'], index=["今日"])    # index名を明後日という文字列に設定
